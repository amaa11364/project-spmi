...
@section('page-icon', 'fa-users-cog')
        ...
        <div class="card-body d-flex justify-content-between align-items-center p-3">
            ...
            <div class="d-flex gap-2 align-items-center">
                <span class="badge" style="background: var(--light-brown); color: var(--primary-brown); padding: 8px 15px; border-radius: 20px;">
                    <i class="fas fa-users me-1"></i> Total: {{ $users->total() }} User
                </span>
                <a href="{{ route('admin.users.export.csv') }}" class="btn" style="background: #28a745; color: white; padding: 8px 15px; border-radius: 8px;">
                    <i class="fas fa-download me-2"></i>Export CSV
                </a>
                <a href="{{ route('admin.users.create') }}" class="btn" style="background: var(--primary-brown); color: white; padding: 8px 18px; border-radius: 8px;">
        ...
        <div class="card-body p-3">
            <form action="{{ route('admin.users.index') }}" method="GET" class="row g-3">
                ...
                <div class="col-md-4 d-flex align-items-end">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="btn" style="background: #e9ecef; color: #495057; padding: 8px 20px; border-radius: 8px;">
                        ...
                        <tr class="border-bottom">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="user-avatar me-2" style="width: 38px; height: 38px; font-size: 14px; {{ $user->id === auth()->id() ? 'background: var(--primary-brown);' : '' }} background: {{ $user->role == 'admin' ? '#dc3545' : '#28a745' }};">
                                        {{ $user->initials }}
                                    </div>
                                    <div>
                            ...
                            <td>
                                <span class="badge" style="background: {{ $user->role == 'admin' ? '#dc3545' : '#28a745' }}; color: white; padding: 6px 12px; border-radius: 20px;">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="small" style="color: {{ $user->is_active ? '#28a745' : '#dc3545' }};">
                            ...
                            <td>
                                <span class="badge" style="background: #e9ecef; color: #495057; padding: 6px 10px;">
                                    ...
                                    {{ $user->dokumens_count ?? $user->dokumens->count() }}
                                </span>
                                @if(($user->dokumens_count ?? $user->dokumens->count()) > 0)
                                    <small class="text-muted d-block mt-1">
                                        {{ $user->formatted_storage_used ?? '0 B' }}
                                    </small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="pe-4">
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" 
                                            ...
                                            data-dokumen-count="{{ $user->dokumens_count ?? 0 }}"
                                            style="background: #f8d7da; color: #721c24; border-radius: 6px; padding: 5px 10px; {{ ($user->dokumens_count ?? 0) > 0 ? 'opacity: 0.5;' : '' }}"
                                            title="Hapus"
                                            {{ ($user->dokumens_count ?? 0) > 0 ? 'disabled' : '' }}>
                            ...
                            <td colspan="8" class="text-center py-5">
                                <div style="opacity: 0.5;">
                                    ...
                                    @if(request('search') || request('role') || request('status'))
                                        ...
                                        <a href="{{ route('admin.users.index') }}" class="btn" style="background: var(--primary-brown); color: white; padding: 8px 20px; border-radius: 8px;">
