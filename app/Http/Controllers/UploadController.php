            ...
            if ($request->jenis_upload === 'file') {
                ...
                if ($request->hasFile('file_dokumen') && $request->file('file_dokumen')->isValid()) {
                    ...
                    if (isset($metadata['penetapan_id'])) {
                        ...
                        return redirect()->route('user.dokumen-saya')
...
public function edit($id)
{
    try {
        ...
        // Hanya dokumen dengan status rejected yang bisa diedit
        if ($dokumen->status !== 'rejected') {
            return redirect()->route('user.dokumen-saya')
                ->with('error', 'Hanya dokumen yang ditolak yang dapat diedit.');
        }
        
        $unitKerjas = UnitKerja::where('status', true)->get();
        $ikus = Iku::where('status', true)->get();
        
        return view('user.edit-dokumen', compact('dokumen', 'unitKerjas', 'ikus'));
    } catch (\Exception $e) {
        return redirect()->route('user.dokumen-saya')
...
public function update(Request $request, $id)
{
    try {
        ...
        // Hanya dokumen dengan status rejected yang bisa diupdate
        if ($dokumen->status !== 'rejected') {
            return redirect()->route('user.dokumen-saya')
                ->with('error', 'Hanya dokumen yang ditolak yang dapat diupdate.');
        }
        ...
        $updateData = [
            ...
            'approved_by' => null,
            'approved_at' => null,
            'admin_note' => null, // Hapus catatan admin
        ];
        ...
        if ($request->hasFile('file_dokumen')) {
            ...
            // Buat folder path
            $folderPath = 'dokumen/' . $request->tahapan . '/' . date('Y');
            $fileName = time() . '_' . Str::random(10) . '.' . $extension;
            $filePath = $file->storeAs($folderPath, $fileName, 'public');
        ...
        } elseif ($request->replace_file == '1' && !$request->hasFile('file_dokumen')) {
            // Jika checkbox replace dicentang tapi tidak ada file yang diupload
            return back()->with('error', 'Silakan pilih file untuk diganti.')
                         ->withInput();
        }
        ...
        // Log aktivitas
        \Log::info('Dokumen diperbaiki oleh user', [
            'user_id' => Auth::id(),
        ...
        return redirect()->route('user.dokumen-saya')
            ->with('success', 'Dokumen berhasil diperbaiki dan menunggu verifikasi ulang oleh admin!');
            
    } catch (\Illuminate\Validation\ValidationException $e) {
        return back()->withErrors($e->validator)->withInput();
    } catch (\Exception $e) {
        \Log::error('Error updating document: ' . $e->getMessage());
        return back()->with('error', 'Gagal mengupdate dokumen: ' . $e->getMessage())
                     ->withInput();
    }
}
    /**
